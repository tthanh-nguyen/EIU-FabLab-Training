...
GPIO_TypeDef *port[] = {GPIOB, GPIOB, GPIOB};
uint16_t pin[] = {LED_R_Pin, LED_B_Pin, LED_G_Pin};
...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
		rgb_blink(&mau,GREEN,1000);
		
		
		//rgb_blink(&rgb, WHITE, 500);
		
//		test_time = HAL_GetTick();
////			// cach 1
//			if (HAL_GetTick() - last_time1 >= 2000)
//			{
...
//					last_time = HAL_GetTick();
		//	}
	//		HAL_GPIO_WritePin(GPIOB, LED_R_Pin, led_state);
			//HAL_GPIO_WritePin(rgb.GPIO_Red, rgb.red_pin, led_state);
			// cach 2
//			if (HAL_GetTick() - last_time >= 100)
//			{
